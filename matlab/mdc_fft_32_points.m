S = [1+1j, 1-1j, -1+1j, -1-1j];
rng(0);  % đảm bảo tái hiện được kết quả
x_all = S(randi(4, 96, 1));  % tạo 96 sample QPSK
x = ifft(x_all(1:32).');

W32 = exp(-1j * 2 * pi/32 * (0:15)).';  % Twiddle factors

%% === Stage 1 ===
x1 = x(1:16);
x2 = x(17:32);
a1 = x1 + x2;
b1 = (x1 - x2) .* W32;
stage1_out = [a1; b1];

%% === Stage 2 ===
stage2_in = stage1_out;
a2 = stage2_in([1:8 17:24]) + stage2_in([9:16 25:32]);
b2 = (stage2_in([1:8 17:24]) - stage2_in([9:16 25:32])) .* W32([1 3 5 7 9 11 13 15 1 3 5 7 9 11 13 15]);
stage2_out = zeros(32,1);
stage2_out([1:8 17:24]) = a2;
stage2_out([9:16 25:32]) = b2;

%% === Stage 3 ===
stage3_in = stage2_out;
a3 = stage3_in([1:4 9:12 17:20 25:28]) + stage3_in([5:8 13:16 21:24 29:32]);
b3 = (stage3_in([1:4 9:12 17:20 25:28]) - stage3_in([5:8 13:16 21:24 29:32])) .* W32(repmat([1 5 9 13],1,4));
stage3_out = zeros(32,1);
stage3_out([1:4 9:12 17:20 25:28]) = a3;
stage3_out([5:8 13:16 21:24 29:32]) = b3;

%% === Stage 4 ===
stage4_in = stage3_out;
a4_idx1 = [1 2 5 6 9 10 13 14 17 18 21 22 25 26 29 30];
a4_idx2 = [3 4 7 8 11 12 15 16 19 20 23 24 27 28 31 32];
a4 = stage4_in(a4_idx1) + stage4_in(a4_idx2);
b4 = (stage4_in(a4_idx1) - stage4_in(a4_idx2)) .* W32(repmat([1 9],1,8));
stage4_out = zeros(32,1);
stage4_out(a4_idx1) = a4;
stage4_out(a4_idx2) = b4;

%% === Stage 5 ===
stage5_in = stage4_out;
a5_idx1 = 1:2:31;
a5_idx2 = 2:2:32;
a5 = stage5_in(a5_idx1) + stage5_in(a5_idx2);
b5 = stage5_in(a5_idx1) - stage5_in(a5_idx2);
stage5_out = zeros(32,1);
stage5_out(a5_idx1) = a5;
stage5_out(a5_idx2) = b5;

%% === Bit-reversal ===
bit_rev_idx = bitrevorder(0:31) + 1;
X_mdc = stage5_out(bit_rev_idx);

%% === So sánh ===
X_ref = fft(x);
fprintf('Max abs error: %.3e\n', max(abs(X_ref - X_mdc)));

% === Vẽ kết quả ===
subplot(2,1,1); stem(real(X_ref), 'bo'); hold on; stem(real(X_mdc), 'rx'); title('Real');
subplot(2,1,2); stem(imag(X_ref), 'bo'); hold on; stem(imag(X_mdc), 'rx'); title('Imag');
